...
namespace ClassLibrary1.DAO
{
    public abstract class CLignePanierDAO : DAO<CLignePanier, int>
    {
        ...
        //recupere toutes les lignes panier dans le BDD
        ...
        //recupere la panier d'un client
        public List<CLignePanier> getByIDClient(int idClient)
        {
            List<CLignePanier> lstAll = new List<CLignePanier>();

            OracleCommand req = new OracleCommand();
            req.Connection = _connex;
            req.CommandText = "SELECT * FROM LIGNE_PANIER WHERE NUMERO_CLIENT = '" + idClient + "'";
